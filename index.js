  ...
  try {
    ...
    const embed = createEmbed(interaction.guild, {
      ...
      description:
        `\`\`\`\n` +
        `ID       : ${targetPlayer.id}\n` +
        `İsim     : ${targetPlayer.name}\n` +
        `Ping     : ${targetPlayer.ping} ms\n` +
        `Steam Hex: ${steamHex}\n` +
        `Lisans   : ${license.slice(0, 20)}...\n` +
        `Discord ID: ${discordId}\n` +
        `\`\`\`\n\n` +
        `🔗 **Discord Hesabı:** ${discordId !== "Yok" ? `<@${discordId}>` : "*Bağlı Değil*"}`
    }).setFooter({ text: "Oyuncunun Yanına Git - Sunucuya hızlı bağlanmak için butonu kullanabilirsin." });

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setLabel(toSmallCaps("sunucuya bağlan")).setStyle(ButtonStyle.Link).setURL(`https://cfx.re/join/${CFX_CODE}`).setEmoji(EMOJI.baglan)
    );
